...
def get_system_info():
	...
	shell_info = "\n".join(
		[read_file_content(os.path.expanduser(f"~/{f}")) for f in shell_config_files]
	)
...
def load_message_store() -> list[list]:
	...
	message_store = [
		[timestamp, messages]
		for timestamp, messages in message_store
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		dest="messages",
		action="store_true",
		help="Print messages; this is for debugging.",
	)
	parser.add_argument(
		"-c", "-clear", dest="clear", action="store_true", help="Clear messages."
	)
	parser.add_argument(
		"-e",
		"-escalate",
	...
	parser.add_argument(
		"-r", "-raw", dest="raw", action="store_true", help="Output raw markdown."
	)
	parser.add_argument(
		"-o", "-ollama", dest="ollama", action="store_true", help="Use local model."
	)
	...
	if args.prompt:  # ask the chatbot a question
		with console.status("[green]Query...", spinner="dots"):
			...
			# Add context from stdin if we have it
			combined_prompt = input_prompt + context
			message_store.append([time(), {"role": "user", "content": combined_prompt}])
			query_response = query(get_messages(message_store))
			if args.raw:
				...
				print_markdown(query_response)
			message_store.append(
				[time(), {"role": "assistant", "content": query_response}]
			)
			save_message_store(message_store)
